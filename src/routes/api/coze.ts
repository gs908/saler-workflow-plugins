...
function parseCozeResponse(resp: Record<string, unknown>) {
  ...
  if (resp[0]) {
    const { executeId, workflowId } = resp;
    const { execute_status, error_code, output, debug_url } = resp[0] as Record<
      string,
      unknown
    >;
    let formatMsg = `executeId:${executeId} in workflow[${workflowId}], status is:${execute_status}.`;
    if (execute_status === "Running") {
      ...
      if (!output) {
        ...
        return {
          success: true,
          data: { executeId, workflowId, status: "success", msg: formatMsg },
        };
      }

      let outputData, markdownContent;
      try {
        outputData = JSON.parse(output as string);
      } catch (e) {
        ...
        try {
          const innerOutput =
            typeof outputData.Output === "string"
              ...
              : outputData.Output;
          markdownContent =
            innerOutput.data || JSON.stringify(innerOutput, null, 2);
        } catch (e) {
          markdownContent = outputData.Output;
        }
      } else {
        markdownContent = JSON.stringify(outputData, null, 2);
      }
      return {
        success: true,
        data: {
      ...
      return {
        success: true,
        msg: formatMsg,
        data: { debug_url, error_code, status: execute_status },
      };
    }
  }
}
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `[API] 查询工作流结果: workflowId=${wfId}, executeId=${execId}`,
      );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const wfId =
        (workflowId as string) || process.env.COZE_MEETING_WORKFLOW_ID || "";
  ...
  try {
    ...
    upload.single("file")(req, res, async (err: any) => {
      if (err) {
        return res
          .status(500)
          .json({ success: false, error: "文件上传失败: " + err.message });
      }
